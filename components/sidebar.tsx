...
export function Sidebar() {
  ...
  const [companyName] = useState("Bill Bridge");
  const [logoUrl] = useState<string | null>("/images/logo.png");
  const [logoLoaded] = useState(true);
  const { sidebarCollapsed, setSidebarCollapsed } = useSidebar();
  const [mobileOpen, setMobileOpen] = useState(false);
  const { t } = useI18n();

  useEffect(() => {
    // Listen for custom event to update company name or logo (if needed elsewhere, but here we keep it fixed)
    const handler = () => {
      // In a hardcoded setup, we might not need to do anything here, 
      // but keeping the listener structure if other parts expect it.
    };
